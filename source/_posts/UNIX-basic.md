...
## 登录(Login)
### 用户名，用户组和密码
/etc/group和/etc/passwd文件，用户给出了group和passwd相关信息，关于这些文件的具体格式可以使用`man 5 passwd`和`man 5 gropy`查看。
...
**关于errno的两条规则:
1. 如果没有出错，errno的值不会被清楚，所以，只有在函数的返回值指明出错的时候，再检验errno的值；
...
## 信号(signal)
Signal用于通知进程发生了某种特殊情况。比如除0操作，将SIGFPE signal发送给进程。对于每一个signal，进程有以下三种方式进行处理：
