...
### 有符号数和无符号数的建议
1. 相同长度的无符号数和有符号数，在运算中有有符号数的话，就会把有符号数转换成无符号数。比如for循环中，`for(unsigned i = 10; i >= 0; --i)`，永远不会跳出`for`循环，因为当$i=0$时，`--i`就相当于得到了有符号数$-1$，而$-1$的补码形式和无符号数的$2^w -1 $的编码是一样的，而这里就是得到了$2^w -1$。
2. 两个无符号数相减。永远不可能小于0。
