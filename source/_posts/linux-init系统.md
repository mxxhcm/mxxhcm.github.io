...
![systemD_para](SystemD_para.jpg)
所有的任务都同时并发执行，总的启动时间被进一步降低为 T1。
- SystemD 提供按需启动能力。
- SystemD采用Cgroup 特性跟踪和管理进程的生命周期。

![find_pid](find_pid.jpg)
...
- 实现事务性依赖关系管理。
- 能够对系统进行快照和恢复。
- 日志service。SystemD Journal 的优点如下：
1. 简单性：代码少，依赖少，抽象开销最小。
2. 零维护：日志是除错和监控系统的核心功能，因此它自己不能再产生问题。举例说，自动管理磁盘空间，避免由于日志的不断产生而将磁盘空间耗尽。
3. 移植性：日志 文件应该在所有类型的 Linux 系统上可用，无论它使用的何种 CPU 或者字节序。
4. 性能：添加和浏览 日志 非常快。
5. 最小资源占用：日志 数据文件需要较小。
6. 统一化：各种不同的日志存储技术应该统一起来，将所有的可记录event保存在同一个数据存储中。
7. 扩展性：日志的适用范围很广，从嵌入式设备到超级计算机集群都可以满足需求。
9. 安全性：日志 文件是可以验证的，让无法检测的修改不再可能。
...
- automount ：自动挂载点
- swap：swap文件
- target ：unit分组统一的控制。比如图形化模式的所有unit组合为一个target。 (例如：multi-user.target 相当于在传统使用 SysV 的系统中运行run level 5)
- timer：定时器
- snapshot ：快照，它保存了系统当前的运行状态。
...
![dependency](recurrent_dependency.jpg)
